...
i = 1;
while i <= TC_total * BD_frac
    RGC_Dors (i) = 0;
    RGC_BD (i) = data(randi(size(data,1)));
    RGC_CART (i) = RGC_Dors (i) + RGC_BD (i);
    CART(i) = data_cart(randi(size(data_cart,1)));
    i = i+1;
end

i = 1;
while i <= TC_total * Dors_frac
    RGC_Dors (end+1) = data(randi(size(data,1)));
    RGC_BD (end+1) = 0;
    RGC_CART (end+1) = RGC_Dors (end) + RGC_BD (end);
    CART(end+1) = data_cart(randi(size(data_cart,1)));
    i = i+1;
end
...
xlabel('Normalized EPSC Amplitude','FontSize',20);
ylabel('Cumulative probability','FontSize',20);
legend([nor_RGC_CART_nonZ_plot nor_CART_nonZ_plot], 'CART_pred', 'CART_expt', 'Location', 'best');
%legend("");
legend boxoff
title("");
